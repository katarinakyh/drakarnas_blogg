...
class TestOfTictactoe extends UnitTestCase {//skapar egen klass som ärver av testsviten
	...
	function testBoardIsFull() {
		$game = new Game();
		$game -> fillMatrixWithValues();

		//$this -> assertIsA($game -> rowIsSame, "BOOLEAN");
		//$this -> assertIsA($game -> colIsSame, "BOOLEAN");
		//$this -> assertIsA($game -> diagonalIsSame, "BOOLEAN");
		$this -> assertTrue($game -> boardIsFull());

	}

	function test3InARow() {
		$game = new Game();
		$game -> fillMatrix();
		$game -> matrix[0][0] = 1;
		$game -> matrix[0][1] = 1;
		...
		$this -> assertTrue($game -> rowIsFull());
